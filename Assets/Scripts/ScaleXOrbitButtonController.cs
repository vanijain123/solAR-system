...
public class ScaleXOrbitButtonController : MonoBehaviour
{
    ...
    void Update()
    {
        if (scaleOrbit == true)
        {
            ...
            if (dist != 0 && start + dist < 2 && start + dist < 0.5)
            {
                ...
                Debug.Log(children);
                autumnWand.objectHit.transform.localScale = new Vector3(currentScale.x + dist, currentScale.y, currentScale.z);
